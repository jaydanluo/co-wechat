...
var tpl = ['<xml>',
  ...
  '<% } else if (msgType === "hardware") { %>',
    '<% if (content && content.sourceKind === "myrank") { %>',
      '<HardWare>',
        '<MessageView><![CDATA[<%-content.sourceKind%>]]></MessageView>',
...
var reply = function (content, fromUsername, toUsername) {
  ...
  if (Array.isArray(content)) {
    ...
    if (content.hasOwnProperty('type')) {
      ...
      if (content.type === 'hardware') {
        return reply2SocialFeatures(fromUsername, toUsername, content.sourceKind);
      }
      type = content.type;
...
/**
 * 接入社交功能消息
 * 详细请看：http://iot.weixin.qq.com/wiki/new/index.html?page=3-4-10
 * @param {String} fromUsername 用户openid
 * @param {String} toUsername 公众号id
 * @param {String} sourceKind 接入数据来源的类型（非官方用词..），值为myrank时代表微信运动榜
 */
var reply2SocialFeatures = function (fromUsername, toUsername, sourceKind) {
  var info = {};
  info.msgType = 'hardware';
  info.createTime = new Date().getTime();   
  info.toUsername = toUsername;
  info.fromUsername = fromUsername;
  info.content = {
